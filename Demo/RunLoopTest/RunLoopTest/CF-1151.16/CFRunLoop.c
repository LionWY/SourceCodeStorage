...
struct __CFRunLoopMode {
    ...
    CFIndex _observerMask;          // observer监听的状态集合
...
struct __CFRunLoop {
    ...
    __CFPort _wakeUpPort;			// 用来唤醒runLoop的端口
...
// 根据runLoop和modename查找runLoop的mode，create 决定是否创建
...
struct __CFRunLoopObserver {
    ...
    CFIndex _order;                 // mode使用数组存储observers，按次序排列的，
...
static void __CFRunLoopObserverSchedule(CFRunLoopObserverRef rlo, CFRunLoopRef rl, CFRunLoopModeRef rlm) {
    ...
    // 如果rlo第一次被添加到rlm，关联对应的rl
    // rlo 可以被加入到多个rlm中，但是关联的rl，只能是第一次关联的rl
    if (0 == rlo->_rlCount) {
        rlo->_runLoop = rl;
    }
    rlo->_rlCount++;
...
struct __CFRunLoopTimer {
    ...
    CFMutableSetRef _rlModes;       // 集合，存放对应的modes，说明一个timers 可以有多个modes，即可以被加入到多个modes中
...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    ...
    if (!__CFRunLoops) {
        ...
        // 可变字典，对应于NSMutableDictionary，
        CFMutableDictionaryRef dict = CFDictionaryCreateMutable(kCFAllocatorSystemDefault, 0, NULL, &kCFTypeDictionaryValueCallBacks);
    
        // 通过pthread_main_thread_np()获取到主线程，然后创建CFRunLoopRef类型的mainLoop   
        CFRunLoopRef mainLoop = __CFRunLoopCreate(pthread_main_thread_np());
        
        // 给上面创建的dict 赋值，key是主线程的指针， value 是刚创建的mainLoop
        ...
        // 这个时候，__CFRunLoops 已经有值了，就是之前创建并赋值的dict
...
#pragma mark - ------ AddItemsToCommonMode -------

// 添加所有的items 到commonMode
...
void CFRunLoopAddCommonMode(CFRunLoopRef rl, CFStringRef modeName) {
    ...
    if (!CFSetContainsValue(rl->_commonModes, modeName)) {
        ...
        if (NULL != set) {
            ...
            // 把所有的items添加进新的mode
            // 
            // 为set中的每个item，调用一次__CFRunLoopAddItemsToCommonMode方法, 其中item是方法的第一个参数
            // context __CFRunLoopAddItemsToCommonMode 的第二个参数ctx
            //  
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    // 如果runLoop停止或者runLoopMode为停止状态，直接返回 kCFRunLoopRunStopped
    ...
    // 判断 是否 线程安全
    Boolean libdispatchQSafe = pthread_main_np() && ((HANDLE_DISPATCH_ON_BASE_INVOCATION_ONLY && NULL == previousMode) || (!HANDLE_DISPATCH_ON_BASE_INVOCATION_ONLY && 0 == _CFGetTSD(__CFTSDKeyIsInGCDMainQ)));
    // 如果是主线程，并且mode是commonMode，端口dispatchPort就是主线程接收消息的端口
    ...
    do {
        ...
        // 创建一个port集合，取值为mode的变量_portSet
        __CFPortSet waitSet = rlm->_portSet;
        
        // runLoop设置为可被唤醒的状态
        __CFRunLoopUnsetIgnoreWakeUps(rl);
        
        // 2. kCFRunLoopBeforeTimers 即将处理Timers， 通知observers
        if (rlm->_observerMask & kCFRunLoopBeforeTimers) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeTimers);
        // 3. kCFRunLoopBeforeSources 即将处理Sources，通知observers
        ...
        // 没有source0事件，且没有超时 poll 为false, 
        // 有source0 事件，或者超时，为true
        Boolean poll = sourceHandledThisLoop || (0ULL == timeout_context->termTSR);

        // didDispatchPortLastTime 初始化为true，所以不会走if方法
        if (MACH_PORT_NULL != dispatchPort && !didDispatchPortLastTime) {
            ...
            // 内部会返回msg，livePort，voucherState，voucherCopy 
        ...
        // didDispatchPortLastTime 设置为false
	...
	if (!poll && (rlm->_observerMask & kCFRunLoopBeforeWaiting)) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeWaiting);
        ...
        // poll 为false ，即没source0，没超时，休眠开始时间就是当前时间
        // poll 为true， 有source0 或者超时，休眠开始时间为0，即不再休眠
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    ...
    // 如果runLoop已经被释放，返回结束状态
    if (__CFRunLoopIsDeallocating(rl)) return kCFRunLoopRunFinished;
    
    // 上锁
    __CFRunLoopLock(rl);
    
    // 获取当前mode，返回的是一个上过锁的mode，内部查找：
    // 1. 在runLoop的 CFMutableSetRef _modes 集合中进行查找
    // 2. 查找不到的情况下， 如果是true，就进行创建，这里是false，只查找不创建
    // 3. 创建 mode 的时候，需要先初始化所有的变量，然后上锁，返回
    CFRunLoopModeRef currentMode = __CFRunLoopFindMode(rl, modeName, false);
    
    // 如果当前mode 不存在，或者当前mode中事件为空，runLoop 结束，返回 kCFRunLoopRunFinished
    ...
    // 声明一个previousMode，赋值为runLoop之前的mode
    CFRunLoopModeRef previousMode = rl->_currentMode;
    
    // 把runLoop的当前mode，设置为即将运行的mode
    rl->_currentMode = currentMode;
    
    // 初始化一个返回结果，值为kCFRunLoopRunFinished
    int32_t result = kCFRunLoopRunFinished;

	// 1. kCFRunLoopEntry， 通知observers 即将开始循环
    if (currentMode->_observerMask & kCFRunLoopEntry ) __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopEntry);
    // 返回值是运行结束之后的结果
	result = __CFRunLoopRun(rl, currentMode, seconds, returnAfterSourceHandled, previousMode);
    
    // kCFRunLoopExit， 通知 observers 即将退出循环runLoop
...
// 这里可以看出，runLoop运行必须指定一个mode，也就是说，当前runLoop只能运行在一个mode中
// 默认是在 kCFRunLoopDefaultMode 中执行的
...
// 用指定的mode启动，并且可以设置runLoop超时时间，是否处理事件后就返回
...
// 添加source 
// 为 runLoop 中的 名字为modeName的mode 添加 source， rls
void CFRunLoopAddSource(CFRunLoopRef rl, CFRunLoopSourceRef rls, CFStringRef modeName) {	/* DOES CALLOUT */
    ...
    // 如果是commonMode，那么commonModes中的所有mode都要更新
    if (modeName == kCFRunLoopCommonModes) {
        // 如果runLoop的commonModes存在，copy一份赋给set
        CFSetRef set = rl->_commonModes ? CFSetCreateCopy(kCFAllocatorSystemDefault, rl->_commonModes) : NULL;
        if (NULL == rl->_commonModeItems) {
            // 如果commonModeItems 不存在，就先初始化
        ...
        if (NULL != set) {
            ...
            // items 更新了，其他的commonMode 也要更新
            // __CFRunLoopAddItemToCommonModes 的第一个参数是set 中的commonMode
            // 第二个参数是context 
        ...
        // 非 common 的 mode
        // 在runLoop中进行查找名字为modeName的mode，找不到就进行创建（true）
        CFRunLoopModeRef rlm = __CFRunLoopFindMode(rl, modeName, true);
        
        // 如果runLoopMode存在，且，集合_sources0为空，就进行初始化创建
        if (NULL != rlm && NULL == rlm->_sources0) {
            rlm->_sources0 = CFSetCreateMutable(kCFAllocatorSystemDefault, 0, &kCFTypeSetCallBacks);
            rlm->_sources1 = CFSetCreateMutable(kCFAllocatorSystemDefault, 0, &kCFTypeSetCallBacks);
            rlm->_portToV1SourceMap = CFDictionaryCreateMutable(kCFAllocatorSystemDefault, 0, NULL, NULL);
        }
        // rlm 存在， _sources0 不包含 rls， _sources1 也不包含 rls
        if (NULL != rlm && !CFSetContainsValue(rlm->_sources0, rls) && !CFSetContainsValue(rlm->_sources1, rls)) {
            // mode对应的 sources0 和sources1 添加 rls， 
            if (0 == rls->_context.version0.version) {
            ...
            __CFRunLoopSourceLock(rls);
            // 如果resource的runLoop不存在，就进行初始化
            if (NULL == rls->_runLoops) {
                
                rls->_runLoops = CFBagCreateMutable(kCFAllocatorSystemDefault, 0, &kCFTypeBagCallBacks); // sources retain run loops!
            }
            // 把rl 加入到resource的runLoop中
    ...
    if (doVer0Callout) {
        ...
        // 解锁之后，把resource 放入source0的schedule
...
void CFRunLoopAddTimer(CFRunLoopRef rl, CFRunLoopTimerRef rlt, CFStringRef modeName) {    
    ...
    if (modeName == kCFRunLoopCommonModes) {
        ...
        // rlt的_rlModes 是否含有rlm的name
        if (NULL != rlm && !CFSetContainsValue(rlt->_rlModes, rlm->_name)) {
           
            __CFRunLoopTimerLock(rlt);
            
            // 如果rlt的_runLoop 不是rl，直接返回，即 rlt只能加入到其对应的_runLoop的mode中
