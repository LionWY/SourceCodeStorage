...
void *_Block_copy(const void *arg) {
    ...
    // 栈上的 block，进行拷贝，并设置 isa 为 malloc
    else {
...
/**
    一个闭包已经被复制，它的fixup例程要求我们修复对共享byref数据的引用
    未复制的闭包必须仍然有效，因此在解除引用转发ptr之后能够始终访问变量
    查询已知的byref指针是否已经被复制到堆中，如果是的话，增加引用计数并返回它
    否则，需要复制它，并更新栈上的转发指针，
    即栈上的 __forwarding 指针指向的是堆上的 Block
...
static struct Block_byref *_Block_byref_copy(const void *arg) {
    ...
    // already copied to heap 如果已经复制到堆上，增加 引用计数，并返回
